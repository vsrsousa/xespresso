...
def render_machine_selector(key="machine_selector", help_text=None):
    ...
    try:
        ...
        default_idx = 0
        if st.session_state.get('current_machine_name') and st.session_state.current_machine_name in machines_list:
...
def render_codes_selector(machine_name, key="codes_selector", help_text=None):
    ...
    try:
        ...
        if not codes_config:
            st.warning(f"⚠️ No codes configured for machine '{machine_name}'. Please configure codes first in the Codes Configuration page.")
            return None
        ...
        if codes_config.versions:
            ...
            if len(available_versions) > 1:
                st.info(f"📦 Multiple QE versions available: {', '.join(available_versions)}")
            
            # Version selector
            default_idx = 0
            if st.session_state.get('selected_code_version') and st.session_state.selected_code_version in available_versions:
                default_idx = available_versions.index(st.session_state.selected_code_version)
            ...
            if selected_version:
                ...
                version_config = load_codes_config(machine_name, DEFAULT_CODES_DIR, version=selected_version)
